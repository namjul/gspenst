...
describe('repository', () => {
  ...
  test('getAll', async () => {
    ...
    expect(result._unsafeUnwrap()).toHaveLength(19)
  })

  test('find', async () => {
    const result1 = await repository.find({ metadata: { slug: 'tag-1' } })
    expect(result1._unsafeUnwrap()).toBeDefined()
    const result2 = await repository.find({ metadata: { slug: 'tag-3' } })
  ...
  test('findAll', async () => {
    ...
    expect(result1._unsafeUnwrap()).toBeDefined()
    expect(result1._unsafeUnwrap()).toHaveLength(19)
